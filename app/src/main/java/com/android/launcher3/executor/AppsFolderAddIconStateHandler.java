...
class AppsFolderAddIconStateHandler extends AbstractAppsStateHandler {
    ...
    private final String mRuleID_94 = "Home_94";

    AppsFolderAddIconStateHandler(ExecutorState stateId) {
        super(stateId);
        ...
        if (this.mRuleID.equalsIgnoreCase("Home_94")) {
            ...
            if (searchResultCount == 0) {
                ...
                if (resultAppName != null) {
                    this.mNlgRequestInfo = new NlgRequestInfo(ExecutorState.APPS_FOLDER_ADD_ICON.toString()).addScreenParam("Text", "Match", "yes").addResultParam("Text", resultAppName);
                }
                ...
                if (checkItemCount < 1) {
                    this.mNlgRequestInfo = new NlgRequestInfo(ExecutorState.APPS_FOLDER_ADD_ICON_SEARCH_RESULT.toString()).addScreenParam("OrdinalNumber", "Exist", "no").addResultParam("apps_count", String.valueOf(searchResultCount));
                } else {
                    getLauncherProxy().addSearchResultItemToFolder(true);
                    this.mNlgRequestInfo = new NlgRequestInfo(ExecutorState.APPS_FOLDER_ADD_ICON.toString()).addScreenParam("SelectedAll", "Exist", "yes").addResultParam("apps_count", String.valueOf(checkItemCount));
                }
            }
        } else if (getLauncherProxy().hasItemInApps(this.mAppInfo)) {
            List<ItemInfo> itemInfoList = getLauncherProxy().getAppsItemInfo(this.mAppInfo);
